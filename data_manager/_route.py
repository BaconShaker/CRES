...
from _gmapper import *
from _pickups import Collection
import urllib
import urllib2
import sys
...
import time
from _get_credentials import get_credentials
...
class Route_Manager(object):
	...
	def run_route(self):
		todays_date = r.strftime("%m/%d/%Y")
		self.ams_lookup()
		try:
			date_picker = cal_prompt()
			
		except Exception, e:
			print "There is no $DISPLAY hooked to this machine. "
			print "I need you to enter the date manually!\n\n"
			print "The correct format is MM/DD/YYYY"
			date_picker = str(raw_input("What date MM/DD/YYYY are you trying to enter? [TODAY]" ))
		
		if len(date_picker) == 0:
			date_picker = str(todays_date)

		print "THIS IS THE DATE YOU PIOCKED!" , date_picker
